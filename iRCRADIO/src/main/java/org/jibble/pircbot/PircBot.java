...
public abstract class PircBot implements ReplyConstants {
    ...
    private TrustAllSSLSocketFactory _socketFactory = null;   
     ...
     */
    public final TrustAllSSLSocketFactory getSocketFactory() {
     ...
     * @throws NickAlreadyInUseException if our nick is already in use on the server.
     */
    public final  void connect(String hostname, int port, String password , TrustAllSSLSocketFactory socketFactory) throws IOException, IrcException, NickAlreadyInUseException, UnknownHostException {
        ...
        if (getEncodingSend() != null) {
            // Assume the specified encoding is valid for this JVM.        	
            outputStreamWriter = new OutputStreamWriter(socket.getOutputStream(), getEncodingSend());
        }
        else {
        ...
        if (getEncodingReceive() != null) {
            // Assume the specified encoding is valid for this JVM.        	
            inputStreamReader = new InputStreamReader(socket.getInputStream(), getEncodingReceive());            
        }
        else {
            // Otherwise, just use the JVM's default encoding.
            ...
            if (secondSpace >= 0) {
                ...
                if (code.equals("004")) {
                    ...
                    if (_autoNickChange) {
                    	tries++;
                        //new line
                        returnNickInUseError(nick);
                        //new shorten nick if over 9 tries
                        if (tries == 10) {                          
                          if (nick.length() > 9  ) { setName( nick.substring(0,7) ); tries = 1; }
                        }
                        nick = getName() + tries;
                        OutputThread.sendRawLine(this, bwriter, "NICK " + nick);
                    }
     ...
     *                                      supported.
     */
    public void setEncodingSend(String charset) throws UnsupportedEncodingException {
        // Just try to see if the charset is supported first...
        "".getBytes(charset);
        
        _charsetSend = charset;
    }

    public void setEncodingReceive(String charset) throws UnsupportedEncodingException {
        // Just try to see if the charset is supported first...
        "".getBytes(charset);
        
        _charsetReceive = charset;
    }
     ...
     *         null if not set.
     */
    public String getEncodingSend() {
        return _charsetSend;
    }
    
    public String getEncodingReceive() {
        return _charsetReceive;
    }
    ...
    public void returnNickInUseError(String message) {
        ...
        if (Globo.actAccountListVisible) {
            ...
            try {
                ...
                data.putString("action", "nickerror");
                data.putString("message", "Nick in use: " + message);
                data.putString("trace", "");
                msg.setData(data);
    ...
    private String _charsetSend = null;
    private String _charsetReceive = null;
